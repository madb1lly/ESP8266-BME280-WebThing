...
// Had to add this to get it to build on Ubuntu w VSCode, wasn't needed on Windows 10 with Arduino IDE
...
// #include <WiFi101.h> Not needed for ESP8266
#include <Thing.h>
...
ThingDevice weather("bme280wbattery", "BME280 Weather Sensor with Battery", bme280Types);
...
ThingProperty batteryVolts("voltage", "", NUMBER, "VoltageProperty");
...
void readBatteryVolts() {
  float volts(NAN);
  volts = analogRead(A0) * 4.5 / 1023;
  ThingPropertyValue value;
  value.number = volts;
  batteryVolts.setValue(value);
}
...
void setup() {
  ...
  // WiFi.beginProvision(); not needed for ESP8266
